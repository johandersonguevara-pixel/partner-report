...
export default function GeneratePage({ user, onGenerated }) {
  ...
  const hasReportLayout = status === 'done' && Boolean(result?.report)

  useEffect(() => {
    if (hasReportLayout) {
      document.body.classList.add('qbr-fullscreen')
    } else {
      document.body.classList.remove('qbr-fullscreen')
    }
    return () => document.body.classList.remove('qbr-fullscreen')
  }, [hasReportLayout])
  ...
  const readyBanner = (
    <div style={{ background:'#f0fdf4', borderRadius:12, border:'1px solid #bbf7d0', padding:'12px 16px', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
      ...
      {hasReportLayout ? (
        <div className="generate-layout">
          ...
          <div className="generate-main">
            <div className="generate-main-inner">
              <div style={{ marginBottom:12 }}>{readyBanner}</div>
              <div className="qbr-report-slot">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'320px 1fr', gap:24, alignItems:'start' }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {status==='done' && result?.report && (
                <div>
                  <div style={{ marginBottom:16 }}>{readyBanner}</div>
                  <QBRReport report={result.report} meta={result.meta} />
                </div>
              )}
